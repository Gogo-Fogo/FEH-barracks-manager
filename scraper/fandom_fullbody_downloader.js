...
const OUT_ROOT = path.join(DB_ROOT, 'unit_assets/fandom/fullbody');
const MANIFEST_DIR = path.join(DB_ROOT, 'unit_assets_manifest/fandom');
const MANIFEST_PATH = path.join(MANIFEST_DIR, 'fullbody_manifest.json');
...
async function resolveStateInfo(fandomBase, stateName) {
  for (const ext of ['webp', 'png', 'jpg', 'jpeg']) {
    const title = `File:${fandomBase} ${stateName}.${ext}`;
    const info = await getImageInfo(title);
    if (info && info.url) return { info, title, ext };
  }
  return null;
}
...
async function main() {
  ...
  for (const hero of selected) {
    ...
    const fandomBase = baseByKey.get(key);
    ...
    let saved = 0;
    for (const [stateName, stateLabel] of states) {
      const resolved = await resolveStateInfo(fandomBase, stateName);
      ...
      const outName = `fandom_${game8Slug}_${stateLabel}.${ext}`;
    ...
    console.log(`${hero.name}: saved ${saved}/4`);
  }

  fs.writeFileSync(MANIFEST_PATH, JSON.stringify(manifest, null, 2));
  console.log(`DONE fullbody items=${manifest.items.length} missing_maps=${manifest.missing_mapping.length}`);
