...
const LIST_API_URL =
  "https://feheroes.fandom.com/api.php?action=parse&page=List_of_Heroes&prop=text&formatversion=2&format=json";
...
async function scrapeFandomRarityMap() {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch fandom list API (${response.status})`);
  }

  const json = await response.json();
  const html = String(json?.parse?.text || "");
  if (!html) {
    throw new Error("Fandom API response did not include parsed list HTML");
  }
